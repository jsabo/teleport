...
variable "aws_iam_policy_name" {
  description = "Name for the AWS IAM policy for resource discovery."
  type        = string
...
variable "aws_organization_iam_policies" {
  ...
  type = object({
    account_enumeration = optional(object({
      name            = optional(string, "teleport-organization-enumeration")
  ...
  validation {
    condition = var.aws_organization_iam_policies.account_enumeration.name == null || can(regex("^[\\w+=,.@-]{1,128}$", var.aws_organization_iam_policies.account_enumeration.name))
  ...
  validation {
    condition = var.aws_organization_iam_policies.join_validation.name == null || can(regex("^[\\w+=,.@-]{1,128}$", var.aws_organization_iam_policies.join_validation.name))
    # Regex can be found at:
    # https://docs.aws.amazon.com/IAM/latest/APIReference/API_Policy.html
