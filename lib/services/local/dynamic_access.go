...
func (s *DynamicAccessService) SetAccessRequestState(ctx context.Context, params types.AccessRequestUpdate) (types.AccessRequest, error) {
	...
	for range maxCmpAttempts {
		...
		if req.GetState() == params.State && (req.GetState().IsDenied() || req.GetState().IsApproved()) {
			return nil, trace.BadParameter("cannot update access request in terminal state %q", req.GetState().String())
		}
