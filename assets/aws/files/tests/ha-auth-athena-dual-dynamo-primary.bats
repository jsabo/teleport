...
TELEPORT_ATHENA_MIGRATION_MODE=dual_dynamo_primary
USE_ACM=false
...
@test "[${TEST_SUITE?}] teleport.storage.audit_events_uri has DynamoDB first" {
    load ${TELEPORT_CONFD_DIR?}/conf
    echo "${TELEPORT_BLOCK?}"
    echo "${TELEPORT_BLOCK?}" | grep -E "^    audit_events_uri: \['dynamodb://"
}

@test "[${TEST_SUITE?}] teleport.storage.audit_events_uri includes Athena" {
    load ${TELEPORT_CONFD_DIR?}/conf
    echo "${TELEPORT_BLOCK?}"
    echo "${TELEPORT_BLOCK?}" | grep -E "athena://"
}
